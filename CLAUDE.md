...
- `Anytime` stores no schedule fields (all null). It never fires an alarm; Home keeps it available to check off until it's checked **once**, after which it's done permanently and never resurfaces (gated on ever-checked, not today's checks).
- `Monthly` stores `MonthlyDayOfMonth` (1..31); months with fewer days clamp to their last day. It never fires an alarm — it's available to check off only on that day each month and (unlike Anytime) resets every month.
...
- The daily preview (`DailyPreviewScheduler`, slots 9/12/18/21 local) lists today's still-open items (timed + Anytime + Monthly). It skips reminders already checked today (Anytime: ever checked) and posts nothing when the list is empty. Each fire chains the next slot.
